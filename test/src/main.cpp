...
namespace TestGame
{
    ...
    public:
        TestGameApp()
        {
            ...
            _targetFPS = GetTargetFPS();
            _fpsLockEnabled = IsFPSLockEnabled();
        }
        ...
        void RenderUI() override
        {
            ...
            // update it once per second so we can read that properly
            float currentFPS = m_deltaTime > 0.0f ? 1.0f / m_deltaTime : 0.0f;

            _fpsUpdateTimer += m_deltaTime;
            if (_fpsUpdateTimer >= _fpsUpdateInterval)
            {
                _displayedFPS = currentFPS;
                _fpsUpdateTimer = 0.0f;
            }

            ImGui::Text("FPS: %.1f (%.3f ms/frame)", _displayedFPS, m_deltaTime * 1000.0f);
            ...
            if (ImGui::Checkbox("Limit FPS", &_fpsLockEnabled))
                SetFPSLockEnabled(_fpsLockEnabled);

            if (_fpsLockEnabled)
            {
                if (ImGui::SliderInt("Max FPS", &_targetFPS, 30, 240))
    ...
    private:
        ...
        float _fpsUpdateInterval = 1.0f; // Update FPS display once per second
    };
}
